...
# URL da API
API_URL="https://registry.unidavi.edu.br/service/rest/v1/search?docker.imageName=universidade-gratuita-back-end"
# Arquivo para armazenar o lastModified
LAST_MODIFIED_FILE="ambiente-minha-unidavi/dev/last_modified/last_modified_universidade_gratuita.txt"
...
check_last_modified() {
    ...
    if [ -f "$LAST_MODIFIED_FILE" ]; then
        ...
        if [ "$current_last_modified" != "$last_saved_last_modified" ]; then
            ...
            "$DOCKER_COMPOSE" -f /home/docker/ambiente-minha-unidavi/dev/docker-compose.yml kill universidade-gratuita-back-end
            "$DOCKER_COMPOSE" -f /home/docker/ambiente-minha-unidavi/dev/docker-compose.yml pull universidade-gratuita-back-end
            "$DOCKER_COMPOSE" -f /home/docker/ambiente-minha-unidavi/dev/docker-compose.yml up -d universidade-gratuita-back-end
        ...
        echo "$current_last_modified" > "$LAST_MODIFIED_FILE"
        echo "Arquivo last_modified_universidade_gratuita.txt criado com o lastModified atual."
        echo "--------------- FIM ---------------"
    fi
}
